...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	calories = ((runningCaloriesMeanSpeedMultiplier * mSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	var calories, mSpeed float64
	mSpeed = math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) // скорость в м/с возведенная в квадрат
	calories = ((walkingCaloriesWeightMultiplier*weight + (mSpeed/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
